...
const Hero = () => {
  ...
  useEffect(() => {
    getTeamsCached().then((structure) => {
      // Alle Teams zählen
      const count = (structure.erwachsene.damen.length || 0)
        + (structure.erwachsene.herren.length || 0)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center">
          ...
          <p className="text-lg text-muted-foreground mb-10 max-w-2xl mx-auto animate-fade-in">
            ...
            {loading ? ' ...' : ` ${teamCount} Mannschaften von den Damen, Herren und der Jugend bis zu den Minis.`}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border-t-4 border-primary">
              ...
              <h3 className="text-2xl font-bold text-foreground mb-2">{loading ? '...' : teamCount}</h3>
